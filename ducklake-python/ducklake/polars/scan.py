...
from ducklake._polars_types import logical_polars_schema, logicalize_polars_expression
...
def scan_ducklake(table: Table, *, include_file_paths: str | None = None) -> pl.LazyFrame:
    ...
    logical_type_casts = [
        logicalize_polars_expression(pl.col(name), logical_schema[name]).alias(name)
        for name, dtype in logical_schema.items()
        if dtype != target_schema[name]
    ]
    if logical_type_casts:
        result = result.with_columns(logical_type_casts)
