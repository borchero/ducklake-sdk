...
impl<T, A> AsyncLazy<T, A> {
    ...
    pub(crate) async fn get_with_arg(&self, arg: A) -> DucklakeResult<&T> {
        self.value.get_or_try_init(|| (self.init)(arg)).await
    }
}

impl<T> AsyncLazy<T, ()> {
    pub(crate) async fn get(&self) -> DucklakeResult<&T> {
